...
unsigned char ReceivedDataBuffer[5];
unsigned char ToSendDataBuffer[5];
...
void UserInit(void)
{
    ...
    OpenUSART(USART_TX_INT_OFF &	// Transmit interrupt off.
			...
			USART_EIGHT_BIT &		// 8 bit transmit/receive width.
           	USART_CONT_RX &			// Continuous reception.
			USART_BRGH_HIGH, 		// High baud rate.				
			155); 					// 19200 bps at Fosc=48 MHz
	TRISC = 0b11111110;
	PORTCbits.RC0 = 0;
...
{   
    ...
    { 
		if (ReceivedDataBuffer[2] == 0x41)		// Cmd = 0x41 is temperature checking command sent from the host.
        {		
			ToSendDataBuffer[0] = 0xAB;
			ToSendDataBuffer[1] = 0x01;
			ToSendDataBuffer[2] = 0x01; // Cmd = 0x01 is temperature
			ToSendDataBuffer[3] = 0x00; //change to value of variable resistor
			ToSendDataBuffer[4] = 0xFF - (0x01 + 0x01 + 0x00);

			if(!HIDTxHandleBusy(USBInHandle))
			{
				USBInHandle = HIDTxPacket(HID_EP, &ToSendDataBuffer[0], 5);
			}
		}

		if (ReceivedDataBuffer[2] == 0x42)		// Cmd = 0x42 is pH checking command sent from the host.
        {
			ToSendDataBuffer[0] = 0xAB;
			ToSendDataBuffer[1] = 0x01;
			ToSendDataBuffer[2] = 0x02; // Cmd = 0x02 is pH value
			ToSendDataBuffer[3] = 0x00; //change to value of variable resistor
			ToSendDataBuffer[4] = 0xFF - (0x01 + 0x02 + 0x00);
		...
		if (ReceivedDataBuffer[2] == 0x43)		// Cmd = 0x43 is humidity checking command sent from the host.
        {
			ToSendDataBuffer[0] = 0xAB;
			ToSendDataBuffer[1] = 0x01;
			ToSendDataBuffer[2] = 0x03; // Cmd = 0x03 is humidity
			ToSendDataBuffer[3] = 0x00; //change to value of variable resistor
			ToSendDataBuffer[4] = 0xFF - (0x01 + 0x03 + 0x00);
        ...
        USBOutHandle = HIDRxPacket(HID_EP,&ReceivedDataBuffer[0],5);  // OUT Endpoint packet size is 5 bytes.
    }
}//end ProcessIO
...
void ProcessSerialIO(void)
{
	...
	if(DataRdyUSART() == 1)
	{
		rx_buffer[serial_count] = ReadUSART();
		serial_count++;
		...
		if(serial_count >= 5)
		{
			...
			// host request for temperature
			if((rx_buffer[0] == 0xAB) && (rx_buffer[1] == 0x02) && 
				(rx_buffer[2] == 0x41) && (rx_buffer[3] == 0x00) && 
				(rx_buffer[4] == (0xFF - 0x02 - 0x41 - 0x00)))
			{
				tx_buffer[0] = 0xAB;
				tx_buffer[1] = 0x01;
				tx_buffer[2] = 0x01; // Cmd = 0x01 is temperature
				tx_buffer[3] = 0x00; //change to value of variable resistor
				tx_buffer[4] = 0xFF - (0x01 + 0x01 + 0x00);

				for(i = 0; i < 5; i++)
				{
					while(BusyUSART());
					WriteUSART(tx_buffer[i]);
				}
			}

			// host request for pH value
			if((rx_buffer[0] == 0xAB) && (rx_buffer[1] == 0x02) && 
				(rx_buffer[2] == 0x42) && (rx_buffer[3] == 0x00) && 
				(rx_buffer[4] == (0xFF - 0x02 - 0x42 - 0x00)))
			{
				
				tx_buffer[0] = 0xAB;
				tx_buffer[1] = 0x01;
				tx_buffer[2] = 0x02; // Cmd = 0x02 is pH value
				tx_buffer[3] = 0x00; //change to value of variable resistor
				tx_buffer[4] = 0xFF - (0x01 + 0x02 + 0x00);
			...
			// host request for humidity
			if((rx_buffer[0] == 0xAB) && (rx_buffer[1] == 0x02) && 
				(rx_buffer[2] == 0x43) && (rx_buffer[3] == 0x00) && 
				(rx_buffer[4] == (0xFF - 0x02 - 0x43 - 0x00)))
			{
				tx_buffer[0] = 0xAB;
				tx_buffer[1] = 0x01;
				tx_buffer[2] = 0x03; // Cmd = 0x03 is humidity
				tx_buffer[3] = 0x00; //change to value of variable resistor
				tx_buffer[4] = 0xFF - (0x01 + 0x03 + 0x00);
